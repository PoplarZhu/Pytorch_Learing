...
% Must be uniformly spaced at some interval dx_in. Must be same length as 'in'.
% x_new = vector of desired sample locations. Must be uniformly spaced at some interval dx_out.
% N = order of interpolating sinc, in units of max(dx_in,dx_out).Must be odd.
% win = 1 if Hamming window applied to interpolation kernel, otherwise no window. (win=1 is recommended.)
%
% OUTPUTS
% out = interpolated data sequence corresponding to sample locations in x_out.
% x_out = sample locations of output vector. This will be a subset of the locations in x_new; relative span of x_new and x_in, and filter
